...
| 4 | Approval tipado não-bypassável: fluxo de `ApprovalQueue` (`src/capability/approval.rs`) — ação que exige aprovação não executa sem resolução; resolução depende de trust (fix do commit e33f841) | `src/capability/approval.rs::tests::compute_hash_is_deterministic_and_input_sensitive`, `::enqueue_or_reuse_twice_reuses_the_same_row`, `::approved_and_executed_row_is_cached_and_never_rerun`, `::approve_and_reject_with_wrong_owner_errors_idor_guard`, `::pending_for_owner_returns_only_that_owners_pending_rows`; `src/capability/registry.rs::tests::needs_approval_true_without_queue_fails_closed`/`_with_queue_queues_instead_of_dispatching`/`_dispatches_after_approval_and_records_executed`/`_false_default_dispatches_immediately_unaffected`; **trust-gate fix (e33f841)**: `src/agent/loop_.rs::tests::approval_resolution_skipped_when_turn_is_untrusted` (an untrusted "sim" must never resolve a pending approval) | covered (existing) |
