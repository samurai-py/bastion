...
- [x] **A-08** Security/live E2E + matriz versionada de targets, capabilities, auth e policy coverage. *(Absorve o antigo UAT-02: o legado não será validado, será substituído.)* **Entregue no Loop 3-F** (`docs/revamp/A-08-security-e2e.md`): suíte de invariantes de segurança do kernel rodada offline (characterization_boundary/capability_registry/fallback_egress_gate/extension_adversarial/extension_ui_adversarial/extension_subprocess/agent_runtime_conformance/agent_runtime_cross_turn_permission/tool_fidelity + egress/approval/registry unit + evals owner-isolation + bastion-memory owner-scope), **zero regressão** (egress/approval/trust/owner-isolation). Live token-free re-verificado ao vivo: `m4_07_unconfigured_auth_profile_fails_closed_before_session_starts` (acpx/claude reais, fail-closed antes de qualquer turn, zero token). Matriz versionada (0.1.0) targets × auth × capabilities × coverage unindo A-05 + `docs/SUPPORT-MATRIX.md`. NÃO-executado marcado honestamente com motivo: turns que gastam token (já provados em A-05/A-06/A-07), `mesh_e2e.sh`/`docker build` (disco/multi-container), codex-via-acpx (mismatch externo), sandbox `Honored` (host sem `bwrap` funcional → `None` fail-closed), GPU/cloud pago (N/A).
- [x] **A-09** Terminal-agent → feature `legacy-terminal-agent` → remoção após 1 release de deprecation (gate: A-05 + A-08 verdes, rollback legado testado). **Loop 3-F entrega a etapa do feature flag (decisão: flag PRIMEIRO, NÃO remover ainda).** `TerminalAgentProvider` (`crates/bastion-providers/src/terminal_agent.rs`) agora só compila sob a feature `legacy-terminal-agent` (OFF por default, tanto em `crates/bastion-providers/Cargo.toml` quanto forwarded do root `Cargo.toml`). Build default não linka o legado + falha fail-closed com erro tipado explicativo em `resolve_provider("claude_code"|"opencode")` apontando pro AgentRuntime (A-03/A-04, substituto provado); build `--features legacy-terminal-agent` ainda compila e resolve. Código NÃO deletado, nota de deprecação em código + `docs/terminal-agent-providers.md`. Testes cobrem os dois estados de feature. **Falta pra remoção plena (item segue aberto ⇒ `[x]` só da etapa flag):** decorrer 1 release de deprecação + rollback do legado testado end-to-end antes de deletar.
...
> **M6-prep entregue no Loop 3-F** (`docs/revamp/M6-PREP.md`): mapa exato Core (as 11 crates em `crates/` — nota: 11, não 10, `bastion-extension-wasm` entrou no 3-C) vs Agent (`src/` + skills + mobile + installer + docker + config), passo-a-passo que o OWNER executa pra criar os 2 repos (git filter-repo com histórico ou cópia + SHA), rewrite de dep path→git-pinned→crates.io, decisões abertas (D-1 destino dos `examples/*`, D-2 destino de `docs/revamp/`, D-3 `STRATEGY.md`), e lista de limpeza item-a-item pro owner aprovar (nada deletado destrutivamente). README ganhou parágrafo Core vs Agent (sem datas). 3 refs `.planning/` mortas removidas de AGENTS.md (reportadas em M6-PREP §5.4). **Nenhum repo criado — owner executa o M6-01..04.**
