...
pub fn load_mcp_config(path: &str) -> anyhow::Result<Value> {
    match std::fs::read_to_string(path) {
        ...
        Err(e) => Err(anyhow::anyhow!("failed to read mcp config at {}: {}", path, e)),
    }
}
...
impl McpClient {
            ...
            let service_result = match transport {
                "stdio" => {
                    ...
                    let args: Vec<String> = server_cfg
                        ...
                        .unwrap_or_default();
                    connect_stdio(&command, &args.iter().map(String::as_str).collect::<Vec<_>>()).await
                }
                "sse" | _ => {
    ...
    pub async fn call_tool_with_timeout(&self, name: &str, args: Value) -> anyhow::Result<Value> {
        ...
        let server = match server {
            Some(s) => s,
            None => anyhow::bail!("server '{}' for tool '{}' not in active connections", server_label, name),
        };
        ...
        match timeout(Duration::from_secs(30), call_future).await {
            Ok(Ok(result)) => {
                Ok(serde_json::to_value(result.content.first())
                    .unwrap_or(Value::Null))
            }
            Ok(Err(e)) => Err(anyhow::anyhow!("tool call failed: {}", e)),
            Err(_elapsed) => {
                Err(BastionError::McpTimeout {
                    tool: tool_name,
                    elapsed_ms: 30_000,
                }.into())
            }
...
async fn connect_stdio(command: &str, args: &[&str]) -> anyhow::Result<RunningService<RoleClient, ()>> {
