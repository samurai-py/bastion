...
async fn compact_skips_when_few_messages() {
    ...
    let result: Vec<_> = ac.compact(&sid, &msgs, &MockProvider as &dyn bastion::provider::Provider, &sm).await.unwrap();
...
async fn compact_fires_with_enough_messages() {
    ...
    struct MockProvider { called: Arc<AtomicBool> }
    #[async_trait::async_trait]
    impl Provider for MockProvider {
        async fn complete(&self, _: &[Message], _: &CallConfig) -> anyhow::Result<LlmResponse> {
            unreachable!()
    ...
    let result: Vec<_> = ac.compact(&sid, &msgs, &mock as &dyn bastion::provider::Provider, &sm).await.unwrap();

    assert!(called.load(Ordering::SeqCst), "complete_simple must be called");
