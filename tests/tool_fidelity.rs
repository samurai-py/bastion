...
fn registry_register_and_lookup() {
    ...
    reg.register("composio", vec!["github_star".into(), "calendar_event".into()]);
...
async fn connect_all_empty_config() {
    ...
    let client = bastion::mcp::McpClient::connect_all(cfg_path.to_str().unwrap()).await.unwrap();
...
async fn connect_all_missing_config() {
    ...
    let client = bastion::mcp::McpClient::connect_all("/tmp/nonexistent-mcp-servers.json").await.unwrap();
    assert!(client.registry().list_tool_names().is_empty());
}
